...
export const PAGES = [
...

  {
    ...
    blocks: [
      ...
      { t: "text", h: "Deep dives", html: "<p>Two engineering case studies come straight out of this product: <a href=\"/work/money-correctness/\" style=\"color:var(--cyan)\">Getting Money Right</a> — the IOU ledger, the two-phase commit against the double-pay race, and refund clawbacks that survive webhook replays — and <a href=\"/work/ping-dispatch/\" style=\"color:var(--cyan)\">The Pings Nobody Answered</a> — diagnosing why 81% of dispatch offers were expiring, with the fix and the funnel data. The operational side lives in <a href=\"/work/ops-console/\" style=\"color:var(--cyan)\">the ~78-page ops console</a>, and the company itself is run with <a href=\"/work/ai-company/\" style=\"color:var(--cyan)\">a 23-agent AI organization</a>.</p>" },
    ],
  },
...
for (const p of PAGES) {
  ...
  for (const b of p.blocks) {
    if (b.t === "text") addPassage(p.title, b.h, url, b.html);
    else if (b.t === "features")
      addPassage(p.title, b.h, url, b.items.map((i) => `${i.title}: ${stripHtml(i.desc)}`).join(" "));
    else if (b.t === "award") addPassage(p.title, "Recognition", url, `${p.title} won: ${b.prize} — ${b.event}.`);
  }
}
