...
const renderBlock = (b) => {
  switch (b.t) {
    ...
    case "raw":
      return b.html;
    case "tabs":
      // Deep-dive tabs: each panel is a full block stack. data-reveal is
      // stripped inside panels — hidden panels would confuse ScrollTrigger.
      return `<section class="proj__section" data-reveal><h2>${b.h}</h2><div class="pk-tabs" id="pk-tabs">
        <div class="pk-tabs__bar" role="tablist">${b.tabs
          .map(
            (t, i) =>
              `<button class="pk-tabs__tab${i === 0 ? " is-on" : ""}" type="button" role="tab" aria-selected="${i === 0}" aria-controls="tab-${t.id}" id="tabbtn-${t.id}" data-tab="${t.id}">${t.label}</button>`
          )
          .join("")}</div>
        ${b.tabs
          .map(
            (t, i) =>
              `<div class="pk-tabs__panel" role="tabpanel" id="tab-${t.id}" aria-labelledby="tabbtn-${t.id}"${i === 0 ? "" : " hidden"}>
          <h3 class="pk-tabs__title">${t.title}</h3>
          <p class="pk-tabs__sub">${t.sub}</p>
          ${t.blocks.map(renderBlock).join("").replace(/ data-reveal/g, "")}
        </div>`
          )
          .join("")}
      </div></section>`;
...
// Parkzy deep dives — rendered as TABS inside the Parkzy page (not
// standalone projects), but still first-class passages in the ask corpus
// and 301-redirect targets for their old /work/<slug>/ URLs (worker.js).
const PK_DIVE_LABELS = { "ai-company": "23-Agent AI Company", "money-correctness": "Getting Money Right", "ping-dispatch": "Ping Dispatch", "ops-console": "Ops Console" };
const PK_DEEP_DIVES_RAW = [
  {
    slug: "ai-company",
    title: "A 23-Agent AI Company",
    category: "AI / ML",
    year: "2026",
    sub: "Parkzy is operated with a 23-seat AI organization: engineering seats, a board with a duty to dissent, and rituals engineered against sycophancy. It has produced 233 versioned PRDs. The agents propose — I decide.",
    tags: ["Agents", "Claude", "Multi-agent", "Ops"],
    links: [
      { label: "Parkzy — the product it runs", href: "/work/parkzy/", internal: true },
      { label: "How I build with AI", href: "/work/how-i-build-with-ai/", internal: true },
    ],
    blocks: [
      { t: "text", h: "The org chart", html: "<p>Twenty-three agent seats, each a written role definition with standing responsibilities: <strong>leadership</strong> (a chief-of-staff CEO seat that synthesizes into decision memos, plus a program manager), a <strong>five-seat board</strong> (market strategist, legal &amp; risk, CFO, a wildcard — and a customer advocate, because the voice of drivers and hosts holds a board seat on purpose), <strong>eleven technical seats</strong> (frontend, backend, iOS, Android, UX, AI, DevOps, QA, data, security, and a collector whose whole job is making sure concerns never evaporate), <strong>trust &amp; safety</strong>, and <strong>four marketing seats</strong>. Premium models take the judgment lanes; cheaper models take the scan-and-draft lanes.</p>" },
      {
        t: "features",
        h: "The rituals",
        items: [
          { title: "Weekly board meeting", desc: "Metrics get refreshed first, then six seats write positions independently — before a clash round where each seat must attack the most dangerous claim on the table. The CEO seat compresses it into a memo of at most three decisions, with dissent preserved verbatim." },
          { title: "Weekly engineering cycle", desc: "Twelve seats report evidence-backed concerns plus one mandatory challenge to another seat's work. A collector triages everything into a task board with a hard WIP cap of 10." },
          { title: "Marketing sprint", desc: "Every one to two weeks, the four marketing seats run the same propose-clash-decide loop on growth work." },
        ],
      },
      { t: "text", h: "The hard problem is sycophancy", html: "<p>A room full of agreeable AI agents is worse than useless — it launders your own biases back to you with confidence. So the mechanisms are the product: a <strong>duty to dissent</strong> written into the charter (“silence is a failed contribution”); <strong>independent-then-adversarial</strong> structure so positions form before they can converge; <strong>standing adversaries</strong> built into the seat definitions — the wildcard generates, the CFO finds the cheapest falsifying test; QA blocks, the CEO seat pushes ship-speed; trust &amp; safety adds friction, the market strategist defends supply; <strong>evidence rules</strong> — no number without a source, and “unknown” is an acceptable answer, which kills the failure mode where confident agents out-argue correct ones; a <strong>dissent ledger</strong> where every decision carries a review-by date and seats earn credibility retroactively for dissents that proved right; and the <strong>collector guarantee</strong> — every raised concern lands on the board, in the backlog, or gets rejected in writing.</p>" },
      { t: "text", h: "What it produces — and who decides", html: "<p>To date: <strong>233 versioned PRDs</strong>, 35 dated decision memos, and shared state files (decision log, experiment queue, risk register, task board) that persist across sessions — the organization has memory. The boundary is explicit in the charter: <strong>no agent makes irreversible business decisions, spends money, or deploys to production on its own.</strong> Every seat advises, drafts, builds, or critiques; my cofounder and I make the calls. That's not a limitation of the system — it's the design. The interesting discovery after months of running it: the value isn't automation, it's <em>structured disagreement on tap</em>.</p>" },
    ],
  },

  {
    slug: "money-correctness",
    title: "Getting Money Right",
    category: "Writing",
    year: "2026",
    sub: "How Parkzy moves real money without losing a cent: an IOU ledger, a two-phase commit against the double-pay race, refund clawbacks that survive webhook replays — and code that screams instead of lying.",
    tags: ["Stripe", "Postgres", "Idempotency", "Case Study"],
    links: [
      { label: "Parkzy — the marketplace", href: "/work/parkzy/", internal: true },
      { label: "The Pings Nobody Answered", href: "/work/ping-dispatch/", internal: true },
    ],
    blocks: [
      { t: "text", h: "The problem", html: "<p>A marketplace takes a driver's money and later pays the host — with cancellations, partial refunds, disputed charges, and Stripe webhooks that can arrive late, twice, or out of order in between. Money bugs are the worst kind: silent, compounding, and trust-destroying. Parkzy defers host payouts (hosts shouldn't have to clear full KYC before their first booking), which means the platform holds <em>debts</em> — and debts need a ledger, not a boolean.</p>" },
      {
        t: "features",
        h: "The machinery",
        items: [
          { title: "An IOU ledger, not a flag", desc: "Every deferred payout is a row: amount (checked positive), the payment intent it came from, the transfer that settles it, a settled-at stamp. A unique index per booking makes backfills idempotent, and row-level security allows no user writes at all — money rows are written only by service-role code." },
          { title: "Two-phase commit vs. the double-pay race", desc: "Phase 1 reserves the row by writing a placeholder transfer id, guarded on “transfer id is null, not settled, amount positive” — so two concurrent workers can't both claim it and a clawed-back row can't transfer. Phase 2 creates the Stripe transfer with a deterministic idempotency key. Phase 3 swaps in the real transfer id. Crash at any point and the retry reuses the same key — Stripe returns the original transfer instead of a second one." },
          { title: "Refund clawbacks that survive replays", desc: "When a refund lands out-of-band, the webhook atomically claims the clawback (a null-guarded stamp, so the webhook and the cancellation path can race and exactly one wins) and clamps the IOU to an absolute target — computed from what was refunded, not multiplied by a ratio — so Stripe redelivering the same webhook is a no-op instead of a double-clamp." },
          { title: "Scream, don't fabricate", desc: "If a transfer succeeds but the ledger update reports zero rows affected, the code refuses to count it as a settle: it logs “money moved with no ledger record” and demands manual reconciliation. The old code counted a fabricated success. The new code's comment says it plainly: if it ever happens, scream instead of lying." },
        ],
      },
      { t: "text", h: "Did it work?", html: "<p>An August 2026 audit reconciled every paid booking and cancellation directly against Stripe. After the fixes and backfills, a clean re-run reported <strong>zero dashboard-vs-Stripe mismatches</strong> on the pending set; 15 payouts totalling $803 settled within 48 hours of maturity; and of the IOUs still unsettled, every matured one was <em>correctly</em> unpayable (canceled booking, demo host, Connect not yet enabled). Honest caveat: that was a one-time audit, not yet a standing monitor — three reconciliation tools (charges, refunds, customers) run on demand, and a repeatable transfers-vs-dashboard report is on the backlog.</p>" },
      { t: "text", h: "What I keep", html: "<p>Nothing here is exotic — it's the boring discipline applied everywhere at once: make illegal states unrepresentable, make every retry idempotent, make race winners explicit, and when reality and the ledger disagree, <strong>prefer the loud failure</strong>. Correctness with money isn't a feature; it's a posture.</p>" },
    ],
  },

  {
    ...
    category: "Writing",
    year: "2026",
    sub: "A real marketplace diagnosis: 81% of dispatch offers were expiring untouched — $655 of demand in two weeks — and the root cause wasn't notifications. A case study in instrumenting before guessing.",
    tags: ["Product Engineering", "Analytics", "Case Study"],
    links: [
      { label: "Parkzy — the marketplace", href: "/work/parkzy/", internal: true },
      { label: "Getting Money Right", href: "/work/money-correctness/", internal: true },
    ],
    blocks: [
      { t: "text", h: "The symptom", html: "<p>Parkzy's core loop is the <em>ping</em>: a driver taps once, every nearby host gets a 60-minute offer, first accept wins. Over one 21-day window, 21 requests went out, <strong>17 expired untouched (81%)</strong> and exactly one was accepted. In the two weeks before the fix, <strong>$655 of offers died on the table</strong> — an average of $40.94 each. For an early marketplace, that's not a leak; that's the boat filling with water.</p>" },
      { t: "text", h: "The investigation", html: "<p>Instinct said push notifications were failing. The data said otherwise: <strong>six of the eight hosts</strong> who let a ping expire were demonstrably <em>inside the app</em> during the 60-minute window. Meanwhile the event funnel showed only ~36% of recipient rows ever produced an in-app “ping received” event. Hosts weren't ignoring the offers — the product was hiding them.</p>" },
      {
        t: "features",
        h: "Three root causes, none of them the obvious one",
        items: [
          { title: "Dismissal was a no-op", desc: "Swiping away the ping sheet did nothing to the underlying offer — the row stayed pending and silently expired 60 minutes later. And once dismissed, the only ways back in were the notification bell or a push deep link." },
          { title: "A self-inflicted TTL bug", desc: "A fix from the week before set the push notification's APNs expiration to 300 seconds — for an offer that lives 60 minutes. Any phone offline for five minutes simply never received the push." },
          { title: "No standing surface", desc: "The driver's side had a persistent banner for the active request. The host's home screen — the side being asked to act — had none." },
        ],
      },
      { t: "text", h: "The fix", html: "<p>Three changes shipped together: a <strong>pending-ping banner on the host's home screen</strong> with a live countdown and the payout amount, one tap from anywhere back into the offer; dismissal copy that tells the host the offer is still live; and push TTL derived from the offer's actual expiry instead of a constant. The diagnosis and fix went from first data pull to production in the same week.</p>" },
      { t: "text", h: "What the data says now", html: "<p>Two structural findings guide what's next. <strong>Speed compounds:</strong> pings accepted within five minutes go on to complete ~69% of the time, versus ~58% for slower accepts — every minute of visibility is worth real conversion. And <strong>dispatch is the next lever, not notifications:</strong> sixteen hosts who never accepted a single ping absorbed 56% of all offers ever sent, while two-thirds of active hosts had never been pinged at all. The lesson I keep: instrument the funnel before believing your own hypothesis — the obvious suspect was innocent, and the fix was product surface area, not infrastructure.</p>" },
    ],
  },

  {
    ...
    category: "Software",
    year: "2026",
    sub: "~78 internal pages, ~36,000 lines of TypeScript: the support and operations platform inside Parkzy — funnels, forensics, risk, disputes, a pricing lab, edge health — gated behind TOTP MFA. The unglamorous software that makes a marketplace operable.",
    tags: ["React", "TypeScript", "Internal Tools", "Ops"],
    links: [
      { label: "Parkzy — the marketplace", href: "/work/parkzy/", internal: true },
      { label: "A 23-Agent AI Company", href: "/work/ai-company/", internal: true },
    ],
    blocks: [
      { t: "text", h: "Why it exists", html: "<p>A two-person company operating a live marketplace can't hire an ops team — it has to <em>build</em> one. Every recurring question became a page: Why did this ping die? Does the dashboard match Stripe? Which hosts are hurting the funnel? Is the edge healthy? The console is where a marketplace stops being a demo and starts being a business you can actually run.</p>" },
      {
        t: "features",
        h: "Representative surfaces (of ~78)",
        items: [
          { title: "Dispatch forensics", desc: "A sent→accepted→booked ping funnel, plus a per-ping forensic timeline that reconstructs exactly what each host saw and when — the tooling behind the ping-dispatch case study." },
          { title: "Money ops", desc: "Revenue, disputes with per-case detail, tax export, and the reconciliation tooling from the money-correctness work." },
          { title: "Risk & trust", desc: "Account risk, listing risk, behavioral signals, overstays and tows — the surfaces that keep a peer-to-peer marketplace safe to transact in." },
          { title: "Growth", desc: "Retention cohorts, attribution, unmet-demand mapping, SEO, host outreach and broadcast messaging." },
          { title: "Platform health", desc: "Feature flags, edge-function health, mobile bundle deploys, and a live AI-spend meter." },
          { title: "Pricing Lab", desc: "A shadow-mode sandbox for the ML pricing engine — compare the model's price against the live price on real inventory before letting it near production." },
        ],
      },
      { t: "text", h: "AI in the loop, humans on the trigger", html: "<p>Support runs AI-assisted, not AI-automated: edge functions triage inbound messages, draft suggested replies, and recommend dispute outcomes — an operator reviews and decides. The console is also where the <a href='#tab-ai-company' style='color:var(--cyan)'>23-agent organization's</a> output lands as concrete dashboards and queues.</p>" },
      { t: "text", h: "The boring parts, done properly", html: "<p>The whole console sits behind <strong>TOTP multi-factor auth</strong> (step-up to AAL2 before any support surface renders), on top of row-level-security boundaries where user-facing roles can't touch money tables at all. Internal tools handle the most sensitive data in the company — they deserve production-grade security more than the marketing site does, not less.</p>" },
    ],
  },
];
const PK_DEEP_DIVES = PK_DEEP_DIVES_RAW.map((p) => ({ id: p.slug, label: PK_DIVE_LABELS[p.slug], title: p.title, sub: p.sub, blocks: p.blocks }));
...
export const PAGES = [
...

  {
    ...
    blocks: [
      ...
      { t: "text", h: "A real ML pricing engine — not an API call", html: "<p>Dynamic pricing runs on a <strong>separately deployed Python service</strong>: a scikit-learn <code>GradientBoostingRegressor</code> behind FastAPI, Dockerized, with Prometheus metrics, health/readiness probes, and endpoints for pricing, hourly curves, retraining, and a booking-outcome feedback loop. It runs in <strong>shadow mode</strong> first — an operator sandbox compares the model's price against the live price on real inventory — and rollout is gated behind a master switch plus a per-host allowlist. An LLM explains the price to users; the <em>number</em> comes from the model.</p>" },
      { t: "tabs", h: "Inside the machine", tabs: PK_DEEP_DIVES },
    ],
  },
...
curl -s https://farazian.com/mcp \\
...

  {
    ...
    category: "AI / ML",
    year: "2026",
    sub: "A question box wired to a real retrieval pipeline over every page on this site: BM25 at the edge, citations back to the source, and — the part that matters — abstention when the corpus can't support an answer. Citations or silence, never a guess.",
    tags: ["RAG", "Retrieval", "BM25", "Abstention", "Evals"],
    links: [
      { label: "Honest — the eval harness", href: "/work/honest/", internal: true },
      { label: "The abstention study", href: "/work/schema-abstention/", internal: true },
    ],
    blocks: [
      {
        t: "raw",
        html: `<section class="proj__section" data-reveal>
        <div class="askd" id="askd" data-demo="ask"></div>
      </section>`,
      },
      { t: "text", h: "How it answers", html: "<p>At build time, the same generator that renders these pages flattens every project, essay, decision record, and resume entry into a passage corpus — one source of truth, so the answers can never drift from the pages you're reading. At query time the Cloudflare Worker runs <strong>BM25 retrieval</strong> over that corpus at the edge, pulls the top passages, and extracts the sentences that actually carry the answer — so every response is <em>verbatim from a cited source</em>, linked back to the page it came from.</p><p>When a model key is configured, a generative tier hands the retrieved passages to Claude to synthesize a cited answer — with a mandatory <code>ABSTAIN</code> escape hatch, and per-IP plus daily spend caps so an open endpoint can't run up a bill. Without a key, the same pipeline answers extractively. Either way: the retrieval, the citations, and the refusal behavior don't depend on an LLM being present.</p>" },
      { t: "text", h: "Abstention is the feature, not a fallback", html: "<p>Most RAG demos are built to always answer — which is exactly how you get confident nonsense. This one scores retrieval confidence and <strong>refuses below threshold</strong>. That design choice comes from research: I ran a <a href='/work/schema-abstention/' style='color:var(--cyan)'>pre-registered study</a> on what happens to honesty when you force models to produce an answer — abstention collapses in every model tested. The fix there is the design here: give the system a first-class way to say “I don't know,” and test that it uses it. The eval suite above runs live against production and asserts both sides: right citations on answerable questions, silence on unanswerable ones.</p>" },
      {
        t: "features",
        h: "What makes it trustworthy",
        items: [
          { title: "One corpus, zero drift", desc: "Passages are generated from the same content literals as the human pages and the MCP server — the build regenerates all three together." },
          { title: "Citations are mandatory", desc: "Every answer carries its sources: title, section, URL, and the verbatim snippet the answer came from. No source, no answer." },
          { title: "Calibrated refusal", desc: "A BM25 score floor plus a matched-term requirement decide when to abstain — and the on-page eval suite pins that threshold in place." },
          { title: "Agents get it too", desc: "The same pipeline is exposed as an <code>ask</code> tool on the MCP server — retrieval-only, since the caller is a model that can synthesize for itself." },
        ],
      },
      { t: "code", h: "It's just HTTP", file: "terminal", code: `curl 'https://farazian.com/api/ask?q=how+does+parkzy+handle+payments'
...
claude mcp add --transport http farazian https://farazian.com/mcp` },
  ...
  {
    slug: "schema-abstention",
    title: "When Models Won’t Say “I Don’t Know”",
    category: "AI / ML",
    year: "2026",
    sub: "A completed, pre-registered study of epistemic abstention under structured output. Force a frontier model to fill a required answer field and honesty collapses — Claude Haiku’s abstention falls from 76% to 2%. An explicit escape hatch only partially repairs it.",
    tags: ["Research", "Pre-registered", "LLMs", "Structured Output", "Evals"],
    links: [
      { label: "Ask This Site — abstention in production", href: "/work/ask/", internal: true },
      { label: "Honest — the eval harness", href: "/work/honest/", internal: true },
    ],
    blocks: [
      { t: "text", h: "The question", html: "<p>Structured output is everywhere — APIs now let you force a model's response into a JSON schema, and production systems increasingly demand it. But a schema with a <em>required</em> answer field leaves no room for the most important thing a model can say: <strong>“I don't know.”</strong> Does enforced structure suppress epistemic honesty? And if you explicitly provide an abstain affordance — a nullable answer plus a status field — do models actually use it?</p>" },
      {
        t: "features",
        h: "The design",
        items: [
          { title: "Pre-registered", desc: "Hypotheses, arms, exclusion rules, and the analysis plan were written and frozen before headline data collection — the way you keep yourself honest about your own honesty study." },
          { title: "Five arms, within-item", desc: "Free-form text → prompted JSON (no enforcement) → API-native strict schema with a required answer → strict schema with a nullable answer + status enum → the same with the status field ordered first." },
          { title: "600 items, contamination-resistant", desc: "300 epistemically unanswerable questions (false premises, fabricated entities, missing context, genuinely unknown answers) + 300 matched answerable controls — mixing legacy anchors with freshly minted items verified to have zero search hits, shipped with a canary GUID." },
          { title: "Four pinned models, three judges", desc: "Four frontier models across three closed families plus a grammar-constrained open model; 12,000 graded responses; a three-judge panel from three model families with Fleiss κ = 0.93 — all three disagreed on just 7 of 2,400 responses." },
        ],
      },
      { t: "text", h: "What it found", html: "<p><strong>Enforced required-answer schemas collapse abstention in every model tested</strong> — risk differences from +0.37 to +0.74, all p&nbsp;&lt;&nbsp;10<sup>−24</sup>. The starkest case: Claude Haiku abstains on 76% of unanswerable questions in free-form text, and <strong>2%</strong> under a required schema. Providing an explicit abstain option (nullable answer + status enum) recovers much of the behavior — but restores parity with free-form honesty for only <em>one of four</em> models. Putting the status field first shows a marginal pooled benefit (+2.8 points, p = .06).</p><p>The one-sentence version: <strong>a required answer field is a hallucination machine, an escape hatch is a partial fix, and the size of “partial” is a property of the model.</strong></p>" },
      { t: "text", h: "Status, honestly", html: "<p>The study is complete: data collected, analysis run, paper drafted, artifact bundle (items, graded responses, judge panel outputs) prepared for release. It is <em>not</em> yet peer-reviewed — the preprint upload is the next step. I'm publishing the findings here the same way the study was run: pre-committed, with the caveats attached.</p><p>And it isn't just a paper to me — the finding is baked into things I ship. <a href='/work/ask/' style='color:var(--cyan)'>Ask This Site</a> abstains by design and is eval-tested for it; <a href='/work/honest/' style='color:var(--cyan)'>Honest</a> treats “refuses when it should” as a first-class check.</p>" },
    ],
  },
  ...
  <body>
    ...
    <main class="proj" id="content" tabindex="-1">
      ...
      <section class="proj__section">
        ...
        ${RESUME_JOBS.map((j) => rsJob(j.role, j.org, j.where, j.when, j.bullets)).join("\n        ")}
      </section>
      <section class="proj__section">
      ...
      <section class="proj__section">
        ...
        ${RESUME_EDU.map((j) => rsJob(j.role, j.org, j.where, j.when, j.bullets)).join("\n        ")}
      </section>
      <section class="proj__section" data-reveal>
        <h2>Skills</h2>
        ${[
    ...
    <main class="proj" id="content" tabindex="-1">
      ...
      <section class="proj__section" data-reveal>
        <div class="st-grid">
          ...
          <div class="st-card"><b id="st-ask">—</b><span>questions answered by <a href="/work/ask/">Ask This Site</a></span></div>
...
const pageSections = (blocks) =>
  blocks
    .flatMap((b) => {
      ...
      if (b.t === "tabs")
        // Flatten deep-dive tabs so agents see the full story via get_project.
        return b.tabs.flatMap((t) => [
          { heading: t.title, text: stripHtml(t.sub) },
          ...pageSections(t.blocks).map((s) => ({ ...s, heading: `${t.title}${s.heading ? " — " + s.heading : ""}` })),
        ]);
      return null;
    })
    .filter(Boolean);
...
// ---- retrieval corpus → public/api/corpus.json ----
// Powers /api/ask (and the MCP `ask` tool): every page flattened into
// passages the worker's BM25 index can ground answers in. Same content
// literals as the pages themselves — the three surfaces cannot drift.
const passages = [];
const addPassage = (title, heading, url, text) => {
  const t = stripHtml(text);
  if (t.length >= 40) passages.push({ id: `p${passages.length}`, title, heading: heading || null, url, text: t });
};

const addBlockPassages = (title, url, blocks) => {
  for (const b of blocks) {
    if (b.t === "text") addPassage(title, b.h, url, b.html);
    else if (b.t === "features")
      addPassage(title, b.h, url, b.items.map((i) => `${i.title}: ${stripHtml(i.desc)}`).join(" "));
    else if (b.t === "award") addPassage(title, "Recognition", url, `${title} won: ${b.prize} — ${b.event}.`);
    else if (b.t === "tabs")
      for (const t of b.tabs) {
        const turl = `${url}#tab-${t.id}`;
        addPassage(t.title, "Overview", turl, `${t.title} (part of Parkzy). ${t.sub}`);
        addBlockPassages(t.title, turl, t.blocks);
      }
  }
};
for (const p of PAGES) {
  const url = `${BASE}/work/${p.slug}/`;
  addPassage(p.title, "Overview", url, `${p.title} (${p.category}, ${p.year}). ${p.sub}`);
  addBlockPassages(p.title, url, p.blocks);
}
addPassage("Milad Farazian", "Profile", `${BASE}/`,
  `Milad Farazian is a full-stack software engineer with AI depth, based in Los Angeles, CA. ${SITE_JSON.headline} ${SITE_JSON.availability} Email: miladfarazian@gmail.com. GitHub: github.com/MiladFarazian. Resume PDF: ${SITE_JSON.links.resume}. Book an intro call: ${SITE_JSON.links.book_intro_call}.`);
...
for (const j of RESUME_EDU)
  addPassage("Resume", "Education", `${BASE}/resume/`, `${j.role}, ${j.org} (${j.where}, ${j.when}): ${j.bullets.join(" ")}`);
addPassage("The Engine Room", "How this site works", `${BASE}/engineering/`,
  "farazian.com is built with Vite and vanilla TypeScript — no framework — with a Three.js GPGPU particle hero (65,536 particles). It runs on a single-file Cloudflare Worker at the edge with KV storage and one Durable Object; there is no origin server and zero databases on the read path. Content is generated at build time by one generator that emits the human pages, the agent-readable site.json, the retrieval corpus, and the sitemap from a single source. Decision records (ADRs) on the engineering page explain each tradeoff: no framework, content as a build step, a raw JSON-RPC MCP server with no SDK, the guestbook stored in KV metadata, live state on a Durable Object, and zero databases on the read path.");
addPassage("The Engine Room", "Postmortem: the Instagram boot hang", `${BASE}/engineering/`,
  "A real production postmortem: visitors from Instagram's in-app browser hung on the boot screen at 00 because the in-app WebView killed an API at module scope, taking down the whole bundle including its own safety net. The fix is three independent defense layers: an inline head watchdog that paints the error onto the boot label and force-dismisses the loader at 8 seconds, crash-isolated subsystem inits with typed fallbacks and a render-loop kill-switch, and a pure-CSS keyframes failsafe. All three were verified headless before deploy.");
addPassage("Status", "Live telemetry", `${BASE}/status/`,
  "The site instruments itself at /status: live visitor presence over WebSockets on a Durable Object, the Cloudflare edge that served the request, the running deploy SHA, MCP call and ask counters, and p75 Web Vitals (LCP, CLS, INP, TTFB) measured from real visitors via a beacon — field data, not lab runs.");
writeFileSync(resolve(ROOT, "public", "api", "corpus.json"), JSON.stringify({ passages }, null, 2) + "\n");
console.log(`generated public/api/corpus.json (${passages.length} passages)`);
